...
namespace alejandro\app\utils;
    ...
    $captchaFont = "Carter.ttf"; 
    $captchaFontSize = rand(30, 40); // Tamaño de la fuente

    $caracter = 0;
    while ($caracter < $totalCharacteres) {
        $captcha .= substr($possiblesLetras, rand(0, strlen($possiblesLetras) - 1), 1); // Se extrae 1 letra aleatoria de $possiblesLetras
        $caracter++;
    }
    ...
    $_SESSION['captchaGenerado'] = $captcha; // Guardamos el captcha generado en la variable de sesión
?>